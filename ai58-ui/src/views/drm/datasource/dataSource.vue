<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="sourceId" width="100" />
      <el-table-column label="数据源名称" align="center" prop="sourceName" :show-overflow-tooltip="true" />
      <el-table-column label="数据源类型" align="center" prop="sourceType" />
      <el-table-column label="连接地址" align="center" prop="connectionUrl" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="数据源名称" prop="sourceName"><el-input v-model="form.sourceName" placeholder="请输入数据源名称" /></el-form-item>
        <el-form-item label="数据源类型" prop="sourceType"><el-input v-model="form.sourceType" placeholder="请输入数据源类型" /></el-form-item>
        <el-form-item label="连接地址" prop="connectionUrl"><el-input v-model="form.connectionUrl" placeholder="请输入连接地址" /></el-form-item>
...
<script>
import { DataSourceApi } from '@/api/drm/dataSource'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      DataSourceApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.sourceId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.sourceId : this.ids[0]
      DataSourceApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.sourceId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => DataSourceApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; DataSourceApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.sourceId ? DataSourceApi.update(this.form) : DataSourceApi.add(this.form)
          action.then(() => { this.$message.success(this.form.sourceId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
