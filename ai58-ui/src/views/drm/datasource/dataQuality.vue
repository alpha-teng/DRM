...
<script>
import { DataQualityApi } from '@/api/drm/dataQuality'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      DataQualityApi.list(this.queryParams).then(res => {
    ...
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; DataQualityApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
