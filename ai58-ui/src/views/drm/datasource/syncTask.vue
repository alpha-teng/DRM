<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="taskId" width="100" />
      <el-table-column label="名称" align="center" prop="taskName" :show-overflow-tooltip="true" />
      <el-table-column label="科室" align="center" prop="deptName" />
      <el-table-column label="数值" align="center" prop="taskValue" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="名称" prop="taskName"><el-input v-model="form.taskName" placeholder="请输入名称" /></el-form-item>
        <el-form-item label="科室" prop="deptId">
        ...
        <el-form-item label="数值" prop="taskValue"><el-input v-model="form.taskValue" placeholder="请输入数值" /></el-form-item>
...
<script>
import { SyncTaskApi } from '@/api/drm/syncTask'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      SyncTaskApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.taskId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.taskId : this.ids[0]
      SyncTaskApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.taskId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => SyncTaskApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; SyncTaskApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.taskId ? SyncTaskApi.update(this.form) : SyncTaskApi.add(this.form)
          action.then(() => { this.$message.success(this.form.taskId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
