<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="executeId" width="100" />
      <el-table-column label="执行年份" align="center" prop="executeYear" :show-overflow-tooltip="true" />
      <el-table-column label="执行月份" align="center" prop="executeMonth" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="执行年份" prop="executeYear"><el-input v-model="form.executeYear" placeholder="请输入执行年份" /></el-form-item>
        <el-form-item label="执行月份" prop="executeMonth"><el-input v-model="form.executeMonth" placeholder="请输入执行月份" /></el-form-item>
        <el-form-item label="预算金额" prop="budgetAmount"><el-input v-model="form.budgetAmount" placeholder="请输入预算金额" /></el-form-item>
...
<script>
import { BudgetExecuteApi } from '@/api/drm/budgetExecute'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      BudgetExecuteApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.executeId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.executeId : this.ids[0]
      BudgetExecuteApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.executeId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => BudgetExecuteApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; BudgetExecuteApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.executeId ? BudgetExecuteApi.update(this.form) : BudgetExecuteApi.add(this.form)
          action.then(() => { this.$message.success(this.form.executeId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
