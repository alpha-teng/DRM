<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="budgetId" width="100" />
      <el-table-column label="预算年份" align="center" prop="budgetYear" :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="预算年份" prop="budgetYear"><el-input v-model="form.budgetYear" placeholder="请输入预算年份" /></el-form-item>
        <el-form-item label="预算类型" prop="budgetType"><el-input v-model="form.budgetType" placeholder="请输入预算类型" /></el-form-item>
...
<script>
import { BudgetEditApi } from '@/api/drm/budgetEdit'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      BudgetEditApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.budgetId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.budgetId : this.ids[0]
      BudgetEditApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.budgetId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => BudgetEditApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; BudgetEditApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.budgetId ? BudgetEditApi.update(this.form) : BudgetEditApi.add(this.form)
          action.then(() => { this.$message.success(this.form.budgetId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
