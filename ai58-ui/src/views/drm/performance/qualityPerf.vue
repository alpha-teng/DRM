<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="不良事件数" align="center" prop="incidentCnt" />
      <el-table-column label="计算日期" align="center" prop="calcDate" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="科室" prop="deptName"><el-input v-model="form.deptName" placeholder="请输入科室" /></el-form-item>
        <el-form-item label="质量得分" prop="qualityScore"><el-input-number v-model="form.qualityScore" placeholder="请输入质量得分" :precision="2" :step="0.1" /></el-form-item>
        <el-form-item label="合格率" prop="qualifiedRate"><el-input-number v-model="form.qualifiedRate" placeholder="请输入合格率" :precision="2" :step="0.1" /></el-form-item>
        <el-form-item label="优秀率" prop="excellentRate"><el-input-number v-model="form.excellentRate" placeholder="请输入优秀率" :precision="2" :step="0.1" /></el-form-item>
        <el-form-item label="不良事件数" prop="incidentCnt"><el-input-number v-model="form.incidentCnt" placeholder="请输入数量" /></el-form-item>
        <el-form-item label="计算日期" prop="calcDate"><el-date-picker v-model="form.calcDate" type="month" value-format="yyyy-MM" placeholder="选择月份" /></el-form-item>
...
<script>
import { QualityPerfApi } from '@/api/drm/qualityPerf'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      QualityPerfApi.list(this.queryParams).then(res => {
    ...
    handleUpdate(row) {
      ...
      QualityPerfApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.id] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => QualityPerfApi.del(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; QualityPerfApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.id ? QualityPerfApi.update(this.form) : QualityPerfApi.add(this.form)
