  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="80px">
      ...
      <el-form-item label="DRG编码" prop="drgCode">
        <el-input v-model="queryParams.drgCode" placeholder="请输入DRG编码" clearable style="width: 200px" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="DRG编码" align="center" prop="drgCode" width="100" />
      <el-table-column label="DRG名称" align="center" prop="drgName" :show-overflow-tooltip="true" />
      <el-table-column label="病例数" align="center" prop="caseCnt" width="100" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="DRG编码" prop="drgCode">
              <el-input v-model="form.drgCode" placeholder="请输入DRG编码" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="病例数" prop="caseCnt">
              <el-input-number v-model="form.caseCnt" :min="0" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script>
import { DrgPerfApi } from '@/api/drm/drgPerf'
...
export default {
  ...
  data() {
    return {
      ...
      dataList: [],
      queryParams: { pageNum: 1, pageSize: 10, calcDate: '', deptName: '', drgCode: '' },
      dialogTitle: '', dialogVisible: false,
      form: {},
      rules: {
        deptName: [{ required: true, message: '科室名称不能为空', trigger: 'blur' }],
        drgCode: [{ required: true, message: 'DRG编码不能为空', trigger: 'blur' }]
      }
    }
  },
  ...
  methods: {
    getList() {
      this.loading = true
      DrgPerfApi.list(this.queryParams).then(res => {
    ...
    handleAdd() { this.reset(); this.dialogTitle = '新增DRG绩效'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const perfId = row ? row.perfId : this.ids[0]
      DrgPerfApi.get(perfId).then(res => { this.form = res.data; this.dialogTitle = '修改DRG绩效'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const perfIds = row ? [row.perfId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => DrgPerfApi.del(perfIds)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; DrgPerfApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.perfId ? DrgPerfApi.update(this.form) : DrgPerfApi.add(this.form)
