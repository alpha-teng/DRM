<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="rbrvsId" width="100" />
      ...
      <el-table-column label="基础点数" align="center" prop="basePoint" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="项目名称" prop="itemName"><el-input v-model="form.itemName" placeholder="请输入项目名称" /></el-form-item>
        <el-form-item label="项目编码" prop="itemCode"><el-input v-model="form.itemCode" placeholder="请输入项目编码" /></el-form-item>
        <el-form-item label="基础点数" prop="basePoint"><el-input v-model="form.basePoint" placeholder="请输入基础点数" /></el-form-item>
...
<script>
import { RbrvsApi } from '@/api/drm/rbrvs'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      RbrvsApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.rbrvsId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.rbrvsId : this.ids[0]
      RbrvsApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.rbrvsId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => RbrvsApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; RbrvsApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.rbrvsId ? RbrvsApi.update(this.form) : RbrvsApi.add(this.form)
          action.then(() => { this.$message.success(this.form.rbrvsId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
