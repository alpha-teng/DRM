<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="reportId" width="100" />
      <el-table-column label="名称" align="center" prop="reportName" :show-overflow-tooltip="true" />
      <el-table-column label="科室" align="center" prop="deptName" />
      <el-table-column label="数值" align="center" prop="reportValue" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="名称" prop="reportName"><el-input v-model="form.reportName" placeholder="请输入名称" /></el-form-item>
        <el-form-item label="科室" prop="deptName"><el-input v-model="form.deptName" placeholder="请输入科室" /></el-form-item>
        <el-form-item label="数值" prop="reportValue"><el-input v-model="form.reportValue" placeholder="请输入数值" /></el-form-item>
...
<script>
import { QualityReportApi } from '@/api/drm/qualityReport'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      QualityReportApi.list(this.queryParams).then(res => {
    ...
    handleUpdate(row) {
      ...
      QualityReportApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.reportId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => QualityReportApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; QualityReportApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.reportId ? QualityReportApi.update(this.form) : QualityReportApi.add(this.form)
