<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="reviewId" width="100" />
      <el-table-column label="点评日期" align="center" prop="reviewDate" :show-overflow-tooltip="true" />
      <el-table-column label="医生姓名" align="center" prop="doctorName" />
      <el-table-column label="处方数" align="center" prop="prescriptionCnt" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="点评日期" prop="reviewDate"><el-input v-model="form.reviewDate" placeholder="请输入点评日期" /></el-form-item>
        <el-form-item label="医生姓名" prop="doctorName"><el-input v-model="form.doctorName" placeholder="请输入医生姓名" /></el-form-item>
        <el-form-item label="处方数" prop="prescriptionCnt"><el-input v-model="form.prescriptionCnt" placeholder="请输入处方数" /></el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.reviewId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.reviewId : this.ids[0]
      PrescriptionApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.reviewId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => PrescriptionApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.reviewId ? PrescriptionApi.update(this.form) : PrescriptionApi.add(this.form)
          action.then(() => { this.$message.success(this.form.reviewId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
