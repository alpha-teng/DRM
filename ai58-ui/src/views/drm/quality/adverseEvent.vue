<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="eventId" width="100" />
      <el-table-column label="发生日期" align="center" prop="eventDate" :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="发生日期" prop="eventDate"><el-input v-model="form.eventDate" placeholder="请输入发生日期" /></el-form-item>
        <el-form-item label="事件类型" prop="eventType"><el-input v-model="form.eventType" placeholder="请输入事件类型" /></el-form-item>
        <el-form-item label="事件等级" prop="eventLevel"><el-input v-model="form.eventLevel" placeholder="请输入事件等级" /></el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.eventId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.eventId : this.ids[0]
      AdverseEventApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.eventId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => AdverseEventApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.eventId ? AdverseEventApi.update(this.form) : AdverseEventApi.add(this.form)
          action.then(() => { this.$message.success(this.form.eventId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
