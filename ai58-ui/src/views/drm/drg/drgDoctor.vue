<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="doctorId" width="100" />
      <el-table-column label="医生姓名" align="center" prop="doctorName" :show-overflow-tooltip="true" />
      <el-table-column label="病种编码" align="center" prop="drgCode" />
      <el-table-column label="CMI值" align="center" prop="cmi" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="医生姓名" prop="doctorName"><el-input v-model="form.doctorName" placeholder="请输入医生姓名" /></el-form-item>
        <el-form-item label="病种编码" prop="drgCode"><el-input v-model="form.drgCode" placeholder="请输入病种编码" /></el-form-item>
        <el-form-item label="CMI值" prop="cmi"><el-input v-model="form.cmi" placeholder="请输入CMI值" /></el-form-item>
...
<script>
import { DrgDoctorApi } from '@/api/drm/drgDoctor'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      DrgDoctorApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.doctorId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.doctorId : this.ids[0]
      DrgDoctorApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.doctorId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => DrgDoctorApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; DrgDoctorApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.doctorId ? DrgDoctorApi.update(this.form) : DrgDoctorApi.add(this.form)
          action.then(() => { this.$message.success(this.form.doctorId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
