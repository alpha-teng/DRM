<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="挂号数" align="center" prop="registerCnt" width="100" />
      <el-table-column label="就诊数" align="center" prop="visitCnt" width="100" />
      ...
      <el-table-column label="平均费用(元)" align="center" prop="avgCost" width="120">
        <template slot-scope="scope"><span>{{ Number(scope.row.avgCost).toFixed(2) }}</span></template>
      </el-table-column>
...
<script>
import { OutpatientApi } from '@/api/drm/outpatient'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      OutpatientApi.list(this.queryParams).then(res => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => OutpatientApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; OutpatientApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
