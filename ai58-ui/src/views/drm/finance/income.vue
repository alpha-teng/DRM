  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      <el-form-item label="收入日期" prop="incomeDate">
        <el-date-picker v-model="queryParams.incomeDate" type="date" value-format="yyyy-MM-dd" placeholder="选择日期" style="width: 150px" />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="收入ID" align="center" prop="incomeId" width="100" />
      <el-table-column label="收入日期" align="center" prop="incomeDate" width="120" />
      <el-table-column label="科室名称" align="center" prop="deptName" width="150" />
      <el-table-column label="收入类型" align="center" prop="incomeType" width="120" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="收入日期" prop="incomeDate"><el-date-picker v-model="form.incomeDate" type="date" value-format="yyyy-MM-dd" placeholder="选择日期" style="width: 100%" /></el-form-item>
        ...
        <el-form-item label="收入类型" prop="incomeType">
          <el-select v-model="form.incomeType" placeholder="请选择收入类型" style="width: 100%">
            <el-option label="门诊收入" value="门诊收入" />
            <el-option label="住院收入" value="住院收入" />
            <el-option label="药品收入" value="药品收入" />
            <el-option label="检查收入" value="检查收入" />
          </el-select>
        </el-form-item>
...
<script>
import { IncomeApi } from '@/api/drm/income'
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      dataList: [],
      queryParams: { pageNum: 1, pageSize: 10, incomeDate: '', deptName: '' },
      dialogTitle: '',
      dialogVisible: false,
      form: {},
      rules: {
        incomeDate: [{ required: true, message: '收入日期不能为空', trigger: 'blur' }],
  ...
  methods: {
    getList() {
      this.loading = true
      IncomeApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.incomeId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.incomeId : this.ids[0]
      IncomeApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.incomeId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => IncomeApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; IncomeApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.incomeId ? IncomeApi.update(this.form) : IncomeApi.add(this.form)
          action.then(() => { this.$message.success(this.form.incomeId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
    reset() {
