<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="warningId" width="100" />
      <el-table-column label="预警标题" align="center" prop="warningTitle" :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="预警标题" prop="warningTitle"><el-input v-model="form.warningTitle" placeholder="请输入预警标题" /></el-form-item>
        ...
        <el-form-item label="预警等级" prop="warningLevel"><el-input v-model="form.warningLevel" placeholder="请输入预警等级" /></el-form-item>
...
<script>
import { AiWarningApi } from '@/api/drm/aiWarning'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      AiWarningApi.list(this.queryParams).then(res => {
    ...
    handleUpdate(row) {
      ...
      AiWarningApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.warningId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => AiWarningApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; AiWarningApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.warningId ? AiWarningApi.update(this.form) : AiWarningApi.add(this.form)
