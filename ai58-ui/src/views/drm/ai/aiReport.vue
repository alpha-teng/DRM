  ...
  <div class="app-container">
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="报告类型" prop="reportType"><el-input v-model="form.reportType" placeholder="请输入报告类型" /></el-form-item>
...
<script>
import { AiReportApi } from '@/api/drm/aiReport'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      AiReportApi.list(this.queryParams).then(res => {
    ...
    handleUpdate(row) {
      ...
      AiReportApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.reportId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => AiReportApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; AiReportApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.reportId ? AiReportApi.update(this.form) : AiReportApi.add(this.form)
