<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="recommendId" width="100" />
      <el-table-column label="推荐标题" align="center" prop="recommendTitle" :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="推荐标题" prop="recommendTitle"><el-input v-model="form.recommendTitle" placeholder="请输入推荐标题" /></el-form-item>
        <el-form-item label="科室" prop="deptName">
          <el-select v-model="form.deptName" placeholder="请选择科室" style="width: 100%">
            <el-option label="内科" value="内科" /><el-option label="外科" value="外科" />
            <el-option label="儿科" value="儿科" /><el-option label="妇产科" value="妇产科" /><el-option label="急诊科" value="急诊科" />
          </el-select>
        </el-form-item>
        <el-form-item label="推荐类型" prop="recommendType"><el-input v-model="form.recommendType" placeholder="请输入推荐类型" /></el-form-item>
...
<script>
import { AiRecommendApi } from '@/api/drm/aiRecommend'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      AiRecommendApi.list(this.queryParams).then(res => {
    ...
    handleSelectionChange(sel) { this.ids = sel.map(i => i.recommendId); this.single = sel.length !== 1; this.multiple = !sel.length },
    handleAdd() { this.reset(); this.dialogTitle = '新增'; this.dialogVisible = true },
    handleUpdate(row) {
      this.reset()
      const id = row ? row.recommendId : this.ids[0]
      AiRecommendApi.get(id).then(res => { this.form = res.data; this.dialogTitle = '修改'; this.dialogVisible = true })
    },
    handleDelete(row) {
      const ids = row ? [row.recommendId] : this.ids
      this.$confirm('是否确认删除选中的数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => AiRecommendApi.remove(ids)).then(() => { this.getList(); this.$message.success('删除成功') })
    },
    handleExport() {
      this.$confirm('是否确认导出所有数据项?', '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => { this.loading = true; AiRecommendApi.export(this.queryParams).then(() => { this.loading = false; this.$message.success('导出成功') }).catch(() => { this.loading = false }) })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const action = this.form.recommendId ? AiRecommendApi.update(this.form) : AiRecommendApi.add(this.form)
          action.then(() => { this.$message.success(this.form.recommendId ? '修改成功' : '新增成功'); this.dialogVisible = false; this.getList() })
        }
      })
    },
